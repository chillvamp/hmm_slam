...
class anchor_body(smach.State):
    ...
    def execute(self,userdata):
        base.follow_trajectory(
            [geometry.pose(x=0.2, y=0.2, ek=.78), geometry.pose(x=0.2, y=0.8, ek=.78),geometry.pose(x=0.4, y=0.8, ek=.78), geometry.pose(x=0.4, y=0.2, ek=0)
            ,geometry.pose(x=0.6, y=0.2, ek=.78), geometry.pose(x=0.6, y=0.8, ek=0.78)]
            , time_from_starts=[5, 10,20,30,40,50], ref_frame_id='map')
