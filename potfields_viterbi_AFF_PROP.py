...
import numpy as np
import pandas as pd
from utils_hmm import viterbi , HMM
...
def readSensor(data):
     ...
     global x,y,th,A,ccxyth,xyth,xythcuant,path,lec
     ...
     if (len(lecs)>buf_vit):
         lecs.pop(0)
         est_vit=viterbi(lecs,Modelo1,Modelo1.PI)
         print('I think IM in',ccxyth[(int)( est_vit[-1:]),:],est_vit[-1:])
     
     graphe= Markov_A_2_grafo(Modelo1.A,ccxyth)        
     ...
     if (len (path)>0):
       ...
       # print ("path",path)
        print('im in ' ,xyth ,xythcuant,len(lecs))
    ...
    #     
     ...
     (x,y),_=quantized(xyth,ccxyth) 
     #(x,y),_=quantized(ccxyth[(int)(est_vit[-1]),:],ccxyth) 
     Fatrx = ( -x + x_nxt)/euclD
     Fatry = ( -y + y_nxt)/euclD
     ...
     Ftotx= Fmag*np.cos(Fth)*.51   +  3*  Fmagat*np.cos(Fatrth)
     Ftoty= Fmag*np.sin(Fth)*.51    + 3*   Fmagat*np.sin(Fatrth)
