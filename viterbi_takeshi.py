...
#        
        ...
        if (len(o_k)>= buf_vit) and (len(o_k2)>= buf_vit):
            ...
            if (len (last_states)>=10 ) and (len (last_states_2)>=10)  :
                ...
                if(last_states[-1]==xyth_odomcuant)  and (last_states_trans[-1]!=last_states_trans[-2])  :
                    ...
                    if  first_adjust_2 :
                        ...
                        xyth_hmms= xyth#ccxyth[last_states[-1]]#xyth#transitions[last_states_trans[0],last_states_trans[1]]
                        xyth_hmms[2]=xyth[2]
                        with  open('dataset_hokuyo_wrs/transitions.txt' , 'a') as out:
                             out.write (str(last_states_trans[-2])+','+str(last_states_trans[-1])+',' +str(xyth[0])+","+ str(xyth[1])  +","+str(euler[2])  +'\n' )
